...
static char* server_ip = "10.0.0.4";
static char* client_ip = "10.0.0.5";
static char* port = "5201";
static size_t do_validation;
static int start_queue = 1;
static int num_queues = 1;
static char* ifname = "enp175s0np0";
static unsigned int ifindex = 4;
static unsigned int iterations = 16;
...
// 	char command[256];
// 	/* Corundum is not gonna work here...*/
...
static void configure_rss(void) {
	...
	memset(command, 0, sizeof(command));
	/* Sets the receive flow hash(rfh) indirection table to spread flows evenly between the first <start_queue> queues. */
	snprintf(command, sizeof(command), "sudo ethtool -X %s equal %d",
		ifname, start_queue);
	printf("Executing RSS CMD: sudo ethtool -X %s equal %d\n", ifname,
		start_queue);
	system(command);
}
...
err_close:
	fprintf(stderr, "[swg]%s(): YNL failed: %s\n", __func__,
		(*ys)->err.msg);
	netdev_bind_rx_req_free(req);
	ynl_sock_destroy(*ys);
	return -1;
...
void run_devmem_tests(void) {
	...
	/* Configure RSS to divert all traffic from our devmem queues */
	configure_rss();
	// printf("Debug: done RSS config...\n");

	// reset_flow_steering();
	// configure_flow_steering();
	// printf("Debug: done flow steering...\n");
	// sleep(1);
	...
	// 	printf("Test: Trying to bind queue<%d>\n", queues[i].idx);
	// }
