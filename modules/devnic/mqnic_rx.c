...
int mqnic_open_rx_ring(struct mqnic_ring* ring, struct mqnic_priv* priv,
    ...
    ring->hdr_buf_len = roundup_pow_of_two(ring->hdr_len);
    ret = mqnic_alloc_hdr(ring);
    if (ret) {
        if (ret == -ENOMEM)
...
void mqnic_close_rx_ring(struct mqnic_ring* ring) {
    ...
    if (ring->buf) {
        //Free up packet buffer(netmem + hdrbuf)
    ...
    if (ring->rx_info) {
        ...
        kvfree(ring->rx_info); //This is an array 
        ring->rx_info = NULL;
    }
...
int mqnic_alloc_hdr(struct mqnic_ring* ring) {
    ring->hdr_bufs_cpu = dma_alloc_coherent(ring->dev, ring->hdr_buf_len * ring->size,
        &ring->hdr_bufs_dma, GFP_KERNEL);
    if (unlikely(!ring->hdr_bufs_cpu)) {
...
void mqnic_free_rx_desc(struct mqnic_ring* ring, int index) {
    ...
    if (!rx_info->pld_netmem)
        return;
    rx_info->hdr_dma_addr = 0;
    rx_info->pld_dma_addr = 0;
    page_pool_put_full_netmem(ring->pp, rx_info->pld_netmem, false);
    rx_info->pld_netmem = 0;
    rx_info->hdr_buf = NULL;
}

int mqnic_free_rx_buf(struct mqnic_ring* ring) {
    u32 index;
    int cnt = 0;
    dma_free_coherent(ring->dev, ring->hdr_buf_len * ring->size,
        ring->hdr_bufs_cpu, ring->hdr_bufs_dma);
...
int mqnic_prepare_rx_desc(struct mqnic_ring* ring, int index) {
    ...
    u8* hdr_buf = ring->hdr_bufs_cpu + index * ring->hdr_buf_len;
    u32 page_order = ring->page_order;
    u32 pld_len = PAGE_SIZE << page_order;
    u32 hdr_len = ring->hdr_len;
    dma_addr_t hdr_dma_addr = ring->hdr_bufs_dma + index * ring->hdr_buf_len;
...
int mqnic_process_rx_cq(struct mqnic_cq* cq, int napi_budget) {
    ...
    while (done < budget) {
        ...
        if (unlikely(!hdr_buf)) {
            netdev_err(priv->ndev, "%s: ring %d null page at index %d; pldnetmem= %lx",
        ...
        */
        ...
        skb = mqnic_skb_copy_header(priv->ndev, &cq->napi, hdr_buf, hdr_len + trim);
        if (unlikely(!skb)) {
