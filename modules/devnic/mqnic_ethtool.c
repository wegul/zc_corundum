...
static void mqnic_get_drvinfo(struct net_device* ndev,
    ...
    strncpy(drvinfo->driver, DRIVER_NAME, sizeof(drvinfo->driver));
    strncpy(drvinfo->version, DRIVER_VERSION, sizeof(drvinfo->version));
    ...
    strncpy(drvinfo->bus_info, dev_name(mdev->dev), sizeof(drvinfo->bus_info));
