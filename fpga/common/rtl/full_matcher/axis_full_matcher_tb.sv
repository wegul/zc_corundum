...
module axis_full_matcher_tb(
  ...
  ***************************************************************************************************/
  ...
  always #17 fclock <= ~fclock;

  always_comb reset_n = ~reset;
  ...
  initial begin
    ...
    ***************************************************************************************************/
    ...
    mst_agent = new("master vip agent",DUT.design_inst.axi4stream_vip_0.inst.IF);
    slv_agent = new("slave vip agent",DUT.design_inst.axi4stream_vip_1.inst.IF);
    ...
    ***************************************************************************************************/
    ...
    mst_agent.driver.seq_item_port.set_max_item_cnt(128);
    ...
    ***************************************************************************************************/
    ...
    fork
      begin
        ...
        // send_a_packet(80*128);
        $display("Customized transaction");
        ...
        for (int i = 0; i < 4; i++)
        begin
        transaction = mst_agent.driver.create_transaction("Master VIP write transaction");
        transaction.set_driver_return_item_policy(XIL_AXI4STREAM_AT_ACCEPT_RETURN );
        assert(transaction.randomize());
        beat = {16'd5, 16'd2, 8'd2, 272'b0};
        keep = '1;
        transaction.set_id(1);
        transaction.set_data_beat(beat);
        transaction.set_keep_beat(keep);
        transaction.set_last(0);
        mst_agent.driver.send(transaction);

        transaction = mst_agent.driver.create_transaction("Master VIP write transaction");
        transaction.set_driver_return_item_policy(XIL_AXI4STREAM_AT_ACCEPT_RETURN );
        assert(transaction.randomize());
        beat = "1706c17a7618b85f9a5024999dede8d417b3819232a748da2d25fa4f9c84";
        keep = '1;
        transaction.set_id(1);
        transaction.set_data_beat(beat);
        transaction.set_keep_beat(keep);
        ...
        transaction.set_data_beat(beat);
        transaction.set_keep_beat(keep);
        transaction.set_last(1);
        mst_agent.driver.send(transaction);
        end

        // for some reason having 9 flits makes tvalid go to x?  I don't even control that how is that possible?
        ...
        transaction.set_driver_return_item_policy(XIL_AXI4STREAM_AT_ACCEPT_RETURN );
        assert(transaction.randomize());
        beat = 'd2; //{16'd6, 16'd3, 8'd2, 272'b0};
        keep = '1;
        transaction.set_id(1);
        transaction.set_data_beat(beat);
        transaction.set_keep_beat(keep);
    ...
    // slv_agent.set_agent_tag("Slave VIP in Slave mode");
    // slv_agent.start_slave();
...
task automatic send_a_packet(xil_axi4stream_uint num_words);
  ...
  bit[128*8-1:0]              data_beat;
  bit[128-1:0]            keep_beat;
  bit[31:0]                               payload; 
  ...
  if((num_words*4)%(128 ) !=0) begin
    $display("Warning, stream VIP TDATA_WIDTH is %d byte, the number of words you wants to pass in a whole packet is %d, the last transfer will be filled with 0",128,num_words );
  end
  ...
  if( (num_words*4)%(128 ) !=0) begin
    total_transfer = (num_words*4)/(128 ) +1;
  end else begin
    total_transfer = (num_words*4)/(128 ) ;
  end

  for(xil_axi4stream_uint i=0; i<total_transfer; i++) begin
    for(xil_axi4stream_uint byte_cnt=0; byte_cnt<(128); byte_cnt++) begin
      data_beat[byte_cnt*8+:8] = byte_mem[byte_cnt+i*(128)];
